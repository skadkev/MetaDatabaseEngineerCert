...
SELECT *
FROM Bookings
WHERE BookingSlot > (SELECT BookingSlot FROM Bookings WHERE GuestLastName = 'McCarthy' AND GuestFirstName = 'Vanessa');
...
SELECT *
FROM MenuItems
WHERE Price = (SELECT Price FROM MenuItems WHERE EXISTS (SELECT * FROM Menus WHERE Menus.ItemID = MenuItems.ItemID AND Cuisine = "Italian") AND Type = "Starters");
